...
WindowManager::WindowManager(ConnectionManager &connectionManager) : running(true), activeWindow(nullptr), connectionManager(connectionManager)
{
...
void WindowManager::addWindow(const std::string& name, std::unique_ptr<IWindow> window)
{
...
void WindowManager::setBackground(const std::string& windowName, const std::string& imagePath) 
{
    SDL_Texture *texture = IMG_LoadTexture(renderer, imagePath.c_str());

    if (!texture) {
        throw std::runtime_error("Erreur lors de la création de la texture de fond " + imagePath);
    }
    ...
    for (const auto& pair : windows) {
        if (pair.second.get() == activeWindow) {
            if (windowTextures.find(pair.first) != windowTextures.end()) {
                SDL_RenderTexture(renderer, windowTextures[pair.first], nullptr, nullptr);
            }
            break;
        }
    }
...
void WindowManager::renderButton(const std::string& text, int x, int y, int w, int h, int r, int g, int b) 
{
    SDL_FRect buttonRect = {static_cast<float>(x), static_cast<float>(y), static_cast<float>(w), static_cast<float>(h)};
