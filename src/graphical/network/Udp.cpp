...
{
    if (sendto(_socket, data.data(), data.size(), 0, (struct sockaddr *)&_server, sizeof(_server)) == -1) {
...
void Udp::receiveFromServer()
{
    ...
    _buffer.resize(4096);
    if (recvfrom(_socket, _buffer.data(), _buffer.size(), 0, (struct sockaddr *)&_server, &serverSize) == -1) {
