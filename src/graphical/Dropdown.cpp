...
Dropdown::Dropdown(float x, float y, float width, float height,
  ...
  _optionButtons.push_back(
      new Button(x, y, width, height, renderer, options[0]));
...
void Dropdown::toggleOpen() { _isOpen = !_isOpen; }
...
void Dropdown::moveButtonToTop() {
  ...
  for (int i = 0; i < _optionButtons.size(); ++i) {
    if (_optionButtons[i]->getText() == _selectedOption &&
        selectedIndex == -1) {
      newX = _optionButtons[i]->getRect()->x;
      newY = _optionButtons[i]->getRect()->y;
      _optionButtons[i]->setPos(x, y);
      selectedIndex = i;
    } else if (_optionButtons[i]->getText() == _formerOption &&
               formerIndex == -1) {
      formerIndex = i;
    }
