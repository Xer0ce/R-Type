...
template <EnemyType T> Entities create_enemy(Registry &r, EnemyProperty enemy) {
  ...
  if constexpr (T == EnemyType::Pion) {
    return create_entity<EntityType::Enemy>(r, Position(random_x, random_y),
                                            Velocity(0, 0), FlatVelocity(5, 5), Health(30),
                                            Draw({}, {}, nullptr), enemy);
  }
  if constexpr (T == EnemyType::Balourd) {
    return create_entity<EntityType::Enemy>(r, Position(random_x, random_y),
                                            Velocity(0, 0), FlatVelocity(2, 2), Health(50),
                                            Draw({}, {}, nullptr), enemy);
  }
  if constexpr (T == EnemyType::Zinzolin) {
    return create_entity<EntityType::Enemy>(r, Position(random_x, random_y),
                                            Velocity(0, 0), FlatVelocity(10, 10), Health(25),
                                            Draw({}, {}, nullptr), enemy);
  }
  if constexpr (T == EnemyType::Boss) {
    return create_entity<EntityType::Enemy>(r, Position(random_x, random_y),
                                            Velocity(0, 0), FlatVelocity(2, 5), Health(100),
                                            Draw({}, {}, nullptr), enemy);
  }
  if constexpr (T == EnemyType::BigBoss) {
    return create_entity<EntityType::Enemy>(r, Position(750, random_y),
                                            Velocity(0, 0), FlatVelocity(0, 2), Health(300),
                                            Draw({}, {}, nullptr), enemy);
  }
}
