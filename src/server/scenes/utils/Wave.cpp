...
void Wave::factory_call(const json& enemy) {
    ...
    std::map<std::string, std::function<void()>> type_map = {
      ...
      {"Balourd", [&]() { create_enemy<EnemyType::Balourd>(*_ecs, it_ai->second); }},
      {"Zinzolin", [&]() { create_enemy<EnemyType::Zinzolin>(*_ecs, it_ai->second); }},
      {"Boss", [&]() { create_enemy<EnemyType::Boss>(*_ecs, it_ai->second); }},
      {"BigBoss", [&]() { create_enemy<EnemyType::BigBoss>(*_ecs, it_ai->second); }},
    };
...
void Wave::load_enemy(const json& enemy) {
    if (enemy.contains("ia") || enemy.contains("number") ||  enemy.contains("type")) {
      if (enemy["number"] < 0)
        return;
      factory_call(enemy);
    } else {
...
void Wave::load(std::string path) {
	...
	if (parsed_file.contains("enemies") && parsed_file["enemies"].is_array()) {
        ...
        std::cout << "Le fichier JSON ne contient pas de tableau 'enemies'." << std::endl;
    }
};