 ...
 * @brief Declaration of the `SparseArray` class, a container for efficiently managing sparse data.
 *
 * This class uses a vector of `std::optional` to store elements, allowing efficient handling of data
 * with potentially large gaps. It provides operations for insertion, access, modification, and erasure.
 */
 ...
 *
 * The `SparseArray` class is a templated container that holds elements of type `Component` for our ECS.
 * It uses a `std::vector` of `std::optional` to store elements, enabling efficient access and management
 * of data with gaps. The class supports insertion, iteration, and erasure operations.
 *
...
class SparseArray {
  public:
    ...
    /** @brief Returns a constant iterator to the beginning of the container (const-qualified). */
    ...
    /** @brief Returns a constant iterator to the end of the container (const-qualified). */
